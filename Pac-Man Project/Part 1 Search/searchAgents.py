...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return tuple(set(state[1])) == tuple(set(self.corners))
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            #print(nextx, nexty)
            if hitsWall == False:
                new_coordinate = (nextx, nexty)
                corners = state[1]
                if new_coordinate in self.corners:
                    corners = list(corners)
                    corners.append(new_coordinate)
                    corners = tuple(set(corners))

                new_state = (new_coordinate, corners)
