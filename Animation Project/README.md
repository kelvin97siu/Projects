...
The Treemap is used as a check to make sure that we are not double adding the same shape. The Treemap is important since its hashing feature will allow us to look up elements instanteously, giving up a O(1) add time. 
